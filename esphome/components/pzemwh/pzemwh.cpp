...
static const char *const TAG = "pzemwh";

static const uint8_t PZEM_CMD_READ_IN_REGISTERS = 0x04;
static const uint8_t PZEM_REGISTER_COUNT = 10;  // 10x 16-bit registers

void pzemwh::on_modbus_data(const std::vector<uint8_t> &data) {
  if (data.size() < 16) {
  ...
  ESP_LOGD(TAG, "PZEM DC: V=%.1f V, I=%.3f A, E=%.1f Wh, P=%.1f W", voltage, current, active_energy, power);
...
void pzemwh::update() { this->send(PZEM_CMD_READ_IN_REGISTERS, 0, 8); }
void pzemwh::dump_config() {
  ESP_LOGCONFIG(TAG, "pzemwh:");
